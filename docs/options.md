...
| `ca_bundle_slot` | `false` | The bundle carries the CA ConfigMap wired to the agent, with `<CA_BUNDLE>` where the PEM goes -- the marker names the option the PEM belongs to, so the file says what is missing on its own. For the common moment: crane is failing TLS and the certificate is still with the platform team. One edit finishes it, and nothing else has to be wired. Refused beside `ca_bundle` -- supplying the PEM and leaving a slot for it are two answers to one question. Every format refuses to *start* on the marker, and each from somewhere different: the chart at `helm install`, the docker bundle from its run script and from compose, and a manifests bundle from a `ca-slot-check` initContainer on the crane Deployment -- `kubectl apply` still creates every object, because a ConfigMap value is not a name, and the pod then stops at `Init:Error`, then `Init:CrashLoopBackOff`. |
