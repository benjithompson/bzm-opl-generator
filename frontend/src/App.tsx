...
import {
  api, downloadZip, saveBlob, Account, AgentStatus, Facts, Feature,
  GeneratedFile, FuncIdChoice, KeyCandidate, Location, Options, Ship,
  SvCheckOut, SvConstants, SvExposeIn, SvExposeOut, SvMocksOut, Workspace,
} from "./api";
import {
...
export default function App() {
  ...
  const pickFeature = useCallback((id: string) => {
    setFeature(id);
  ...
  useEffect(() => {
    ...
    if (!facts) { if (!feature) pickFeature(features[0].id); return; }
    const start = startFeature(facts.func_ids, features);
    if (start) pickFeature(start);
